...
*/
 ...
 set<ll> st; 
 vector <ll> dijkstra(ll V, vector<vector<pair<ll,ll>>> &adj, ll S)
    {
        set<pair<ll,ll>> s;
        vector<ll> dis(V,1e9);
...
void solve()
{
   ...
   for(ll i=0;i<n;++i)
   {
    vector<ll> s_distance=dijkstra(n,adj,i);
    for(ll j=0;j<s_distance.size();++j)
    {
        if(s_distance[j]!=1e9)
        temp_sum+=s_distance[j];
    }
   }
   ...
   for(ll i=0;i<n;++i)
   {
    ...
    for(ll j=0;j<s_distance.size();++j)
    {
        if(s_distance[j]!=1e9 && st.find(i)==st.end() && st.find(j)==st.end())
        temp_sum+=s_distance[j];
    }
   }
