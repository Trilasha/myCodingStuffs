...
void solve()
{
    ...
    vector<pll> vp;
    fr(i,t)
    vp.pb({max(0ll,v[i]-r),min(n,v[i]+r)});
    ...
    for(ll i=0;i<t;++i)
    {
        while(temp.size()>=2 && temp[temp.size()-2].ss>=vp[i].ff)
        temp.pop_back();

        temp.pb(vp[i]);

        if(temp[temp.size()-1].ss>=n)
        break;
    }

    for(ll i=1;i<temp.size()-1;++i)
    {
        if(temp[i].ss<temp[i+1].ff)
        {
    ...
    if(temp[1].ff>0 || temp[temp.size()-1].ss<n)
    {
        cout<<"IMPOSSIBLE"<<endl;
        return;
    }
