...
void solve()
{
    ...
    fr(i,x)
    {
        ll ind=lower_bound(all(v1),v2[i])-v1.begin();
        //cout<<ind<<endl;
        if(ind==0)
         ans.pb(pref[n-1]-(v2[i]*n));
        else if(ind==n)
         ans.pb((v2[i]*n)-pref[n-1]);
        else if(v1[ind]==v2[i])
        {
            ll cal= (pref[n-1]-pref[ind-1])-(n-ind)*v2[i];
            ll cal2=(ind+1)*v2[i] - pref[ind];
            ans.pb(cal+cal2);
        }
        else
        {
