 ...
 int findMinDifference(vector<string>& timePoints) {
        ...
        for(int i = 0; i < timePoints.size(); i++){
            if(timePoints[i] == "00:00"){
                ...
                int r1 = timePoints[i][0] - '0';
                int r2 = timePoints[i][1] - '0';
                int b1 = timePoints[i][3] - '0';
                int b2 = timePoints[i][4] - '0';
                time.push_back((r1*10+r2)*60 + (b1*10+b2));
            }                                    
        }
        dbg(time);
        ...
        for(int j = 1; j < time.size(); j++){
            diff.push_back(time[j] - time[j-1]);
        }
        dbg(diff);
        diff.push_back(time[0] + 24*60 - time[time.size()-1]);
        dbg(diff);
