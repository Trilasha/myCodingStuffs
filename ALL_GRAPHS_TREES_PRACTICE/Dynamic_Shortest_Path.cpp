...
void solve()
{
    ...
    vector<ll> dp1=dijkstra(adj,n,s);
    vector<ll> dp2=dijkstra(adj,n,t);
    ll original=dp1[t];
    // printv(dp1);
    // printv(dp2);
    fr(i,qu)
    {
        ...
        //s y x t;
        ll diss_x=min(dp1[x],dp1[y]+wt);
        ll fromt=dp2[x];
        ll curr1=diss_x+fromt;


        //s x y t
        ll diss_y=min(dp1[y],dp1[x]+wt);
        ll fromtt=dp2[y];
        ll curr2=diss_y+fromtt;

        ll ans=min({original,curr1,curr2});
