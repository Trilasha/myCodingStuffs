...
bool cmps(pair<string,ll> &a,pair<string,ll> &b)
{
    if(a.first!=b.first)
...
bool cmpdur(pair<ll,ll> &a,pair<ll,ll> &b)
{
   if(a.first!=b.first)
    return a.first<b.first;
    else
...
void solve()
{
    inp(n);
    ll ori=n;
        vector<pair<string,ll>> vps;
        vector<pair<ll,ll>> vpdur;
    while(n--)
    {
        string s;
        cin>>s;
        ...
        cin>>dur;
        cin>>id;
        pair<string,ll> p1=make_pair(s,id);
        pair<ll,ll> p2=make_pair(dur,id);
        vps.pb({s,id});
        vpdur.pb({dur,id});
    }
    sort(vps.begin(),vps.end(),cmps);
    // for(auto &i:vps)
    // cout<<i.first<<" "<<i.second<<endl;
     sort(vpdur.begin(),vpdur.end(),cmpdur);
    //  for(auto &i:vpdur)
    // cout<<i.first<<" "<<i.second<<endl;
     ll ct=0;
     for(ll i=0;i<ori;++i)
     {
       if(vps[i].second==vpdur[i].second)
        ct++;
     }
     cout<<ct<<endl;
}
