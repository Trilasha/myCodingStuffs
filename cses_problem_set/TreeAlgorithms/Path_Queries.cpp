...
//~subtree queries but in this case insteade of storing the subtree size, we are storing the rightmost index of the
//subtree of a node ---> observe the difference in the endd vector
...
void solve(){
    ...
    for(ll i=1;i<=n;++i){
        ...
        bt.update(endd[i]+1,-val[i],n); //to cancel the effect beyond its own subtree
    }
    ...
    fr(i,q){
        ...
        if(type==1){
            ...
            bt.update(endd[node]+1,-(-val[node]+new_val),n);
            val[node]=new_val;
        }else{
            ll node;
            cin>>node;
            cout<<bt.query(start[node])<<endl;
        }
    }
}
