...
map<ll,vector<ll>> mp;
vector<ll> ans;
bool detectCycle(vector<vector<ll>> &adj,ll node,ll par,ll &des,vector<ll> &vis)
{
    vis[node]=1;
    for(auto &i:adj[node])
    {
        if(!vis[i])
        {
            if(detectCycle(adj,i,node,des,vis))
            return true;
        }
...
// void dfs(vector<vector<ll>> &adj,ll node,ll par,vector<ll> &ans)
// {
//     ans.pb(node);
//     vis[node=1];
//     for(auto &i:adj[node])
...
void solve()
{
    ...
    for(ll i=1;i<=n;++i)
    {
        if(!vis[i])
        {
            if(detectCycle(adj,i,-1,des,vis))
               break;
        }
    }
