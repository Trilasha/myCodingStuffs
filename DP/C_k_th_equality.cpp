...
void solve(){
    ...
    //cout<<str<<" "<<rngA<<endl;
    for(ll i=str;i<=rngA;++i){
        ll min_b=max(bpow(10,c-1)-i,bpow(10,b-1));
        ll max_b=min(bpow(10,c)-i,bpow(10,b));
        //cout<<i<<" "<<cnt<<" "<<req<<" "<<sum<<"("<<min_b<<" "<<max_b<<")"<<endl;
        cnt=max_b-min-b;
        if(cnt<=req){
            req-=cnt;
        }else{
    ...
    //cout<<A<<low_b<<high_b<<endl;
    if(A==-1){
        cout<<-1<<endl;
        return;
    }

    for(ll i=low_b;i<=high_b;++i){
        req--;
        ll val=A+i;
