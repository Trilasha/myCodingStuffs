...
//tc--> O(n*k) ~ 10^7
//earlier one was giving tle beacause it's tc was O(n*h) ~ 10^9
//topdown approach followed by the bottomup approach

vector<ll> dp(1e6,INT_MAX);
ll recur(ll n,ll k,vector<ll> &height)
{
    if(n==0)
    return 0;
    ...
    {
        if(n-i>=0)
        ans=min(ans,abs(height[n]-height[n-i])+recur(n-i,k,height));
    }
