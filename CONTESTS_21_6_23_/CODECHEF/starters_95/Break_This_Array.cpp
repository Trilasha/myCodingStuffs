...
ll recur(ll ind,ll left,ll right){
    if(ind==k){
    ...
    if(s[ind]=='L'){
        for(ll i=left;i<right;++i){
            ans=mod_add(ans,recur(ind+1,left,i),modval);
        }
    }else{
        for(ll i=left+1;i<=right;++i){
            ans=mod_add(ans,recur(ind+1,i,right),modval);
        }
    }
    ans=mod_div(ans,right-left,modval);
    return dp[ind][left][right]=ans;
}
...
void solve(){
    ...
    dp.assign(k+1,vector<vector<ll>>(n+1,vector<ll>(n+1,-1)));
