...
void solve()
{
    ...
    //all -ve to left and +ve to right
    while(!bb.empty())
    {
        if(bb.front()<0)
        {
            aa.push_front(bb.front());
            bb.pop_front();
        }
        else
        {
      ...
      while(!bb2.empty())
    {
        if(bb2.front()<0)
        {
            aa.push_back(bb2.front());
            bb2.pop_front();
        }
        else
        {
            aa.push_front(bb2.front());
            bb2.pop_front();
        }
    }
    vector<ll> ans2;
    ...
    // printv(ans2);
    //
    ll ct1=maximum_subarray_sum(ans1);
    ll ct2=maximum_subarray_sum(ans2);
