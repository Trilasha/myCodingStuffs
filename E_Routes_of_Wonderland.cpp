...
vector<ll> dijkstra(vector<vector<pair<ll,ll>>> &adj,ll n,ll S)
{
    ...
    while (!pq.empty())
    {
        ...
        for (auto &i : adj[node])
        {
            ...
            if(chk==edgeWeight)
            {
                if(distance<dis[childNode])
                {
                      dis[childNode] = distance;
                      pq.push({dis[childNode], {chk,childNode}});
                      parent[childNode] = node;
                }
            }
            else
            {
                if(distance+1<dis[childNode])
                {
                      dis[childNode] = distance+1;
                      pq.push({dis[childNode], {chk^1,childNode}});
                      parent[childNode] = node;
                }
            }
        }
...
void solve()
{
    ...
    ll x,y;
    cin>>x>>y;
    vector<ll> ans=dijkstra(adj,n,x);
    cout<<ans[y]<<endl;
}
