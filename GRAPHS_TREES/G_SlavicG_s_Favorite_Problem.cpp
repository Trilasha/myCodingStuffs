...
set<ll> fromb;
ll a,b;
void dfs1(ll node,vector<vector<pll>> &adj,ll par,ll xx)
{
    for(auto &i:adj[node])
    {
        ll c_node=i.ff;
        ll wt=i.ss;
        if(c_node!=par && c_node!=b)
        {
            froma.insert(xx^wt);
            dfs1(c_node,adj,node,xx^wt);
        }
    }
}


void dfs2(ll node,vector<vector<pll>> &adj,ll par,ll xx)
{
    for(auto &i:adj[node])
    {
        ll c_node=i.ff;
