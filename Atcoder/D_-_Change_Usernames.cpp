...
//replacing the vector things with map --> problem solved 


bool detect_cycle(ll n,string s,map<string,vector<string>> &mp,map<string,ll> &vis,map<string,ll> &pathVisited)
{
    vis[s]=1;
    pathVisited[s]=1;
    ...
    for(auto &child:mp[s])
    {
        ...
        if(!vis[child])
        {
            if(detect_cycle(n,child,mp,vis,pathVisited))
            return true;
        }
    }
...
void solve()
{
    ...
    for(ll i=0;i<n;++i)
    {
        if(!vis[v[i]])
        {
            if(detect_cycle(n,v[i],mp,vis,pathVisited))
            {
                cout<<"No"<<endl;
                return;
            }
