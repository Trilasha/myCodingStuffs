...
void solve(){
    ...
    inpv(v);
    sort(all(v));
    vector<vector<ll>> dp(n+1,vector<ll>(5002,-1));
    function<ll(ll,ll)> recur=[&](ll i,ll sum){
        if(i==n){
            return 0LL;
        }
        if(dp[i][sum]!=-1LL){
        ...
        ll ans=0;
        ll add=max(v[i],(v[i]+sum+1)/2);
        ans=(add+recur(i+1,sum+v[i]))%md;
        ans=(ans+recur(i+1,sum))%md;
        return dp[i][sum]=ans;
    };
    cout<<recur(0,0)%md<<endl;
}
