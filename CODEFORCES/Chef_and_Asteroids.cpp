...
ll expo(ll a, ll b, ll modvald) {ll res = 1; while (b > 0) {if (b & 1)res = (res * a) % modvald; a = (a * a) % modvald; b = b >> 1;} return res;}
...
void solve(){
    ...
    ll num=1;
    for(auto &i:mp){
        num=(num*f[i.ss])%modval;
    }
    //cout<<num<<endl;

    ll den=f[n];
    //cout<<inverse(den)<<endl;
    cout<<(num*inverse(den))%modval<<endl;
}
