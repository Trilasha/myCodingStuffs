...
void solve()
{
    ...
    fr(i,m)
    {
        ll pos,val;
        cin>>pos>>val;
        cnt[v[pos-1]]+=((i+1)-appear[v[pos-1]]);
        appear[v[pos-1]]=-1;
        appear[val]=(i+1);
        v[pos-1]=val;
    }
    ...
    // for(auto &i:mp)
    // cout<<i.ff<<"-->"<<cnt[i.ff]<<endl;
    
    ll ans=0;
    for(auto &i:appear)
    {
        ll where_val_doesnt_occur=m-cnt[i.ff];
        ll temp=where_val_doesnt_occur;

        ans+=((m*(m+1))/2 - (temp*(temp+1))/2);
    }
