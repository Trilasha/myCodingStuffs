...
ll recur(ll ind,ll xro){
    if(ind>=n){
        return ((xro==0)?0:-1e11);
    }
    if(dp[ind][xro]!=-1) return dp[ind][xro];
    ll pick=v[ind]+recur(ind+1,(xro^v[ind]));
    ll notpick=recur(ind+1,xro);
    return dp[ind][xro]=max(pick,notpick);
}
void solve(){
    cin>>n;
    v.resize(n);
    inpv(v);
    dp.assign(n+1,vector<ll>(1024,-1));
    cout<<max(0ll,recur(0,0))<<endl;
}
