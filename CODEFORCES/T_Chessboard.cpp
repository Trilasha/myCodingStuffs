...
void solve()
{
    ...
    vector<vector<ll>> one(n,vector<ll>(n));
    vector<vector<ll>> zero(n,vector<ll>(n));
    ...
    ll ans=vp[0].ff+vp[1].ff+vp[2].ss+vp[3].ss;

    sort(all(vp),cmp);

    ans=min(ans,vp[0].ss+vp[1].ss+vp[2].ff+vp[3].ff);
