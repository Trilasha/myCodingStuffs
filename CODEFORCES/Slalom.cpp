...
ll recur(ll ind,ll level){
    ...
    if(level==(n-1)){
        return dp[level][ind]=v[level][ind];
    }

    return dp[level][ind]=v[level][ind]+max(recur(ind,level+1),recur(ind+1,level+1));
}
...
void solve(){
    ...
    dp.resize(n+1,vector<ll>(n+1,neg));
    cout<<recur(0,0)<<endl;
}
