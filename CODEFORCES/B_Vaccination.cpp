...
void solve()
{
    ll n,cap,vac_lim,per_lim;
    cin>>n>>cap>>vac_lim>>per_lim;
    ...
    ll ans=1;
    ll curr=0;
    ll temp_low=v[0]+per_lim;
    ll temp_high=temp_low+vac_lim;
    ...
    for(ll i=0;i<n;++i)
    {
        if(curr==cap)
        {
            ans++;
            curr=1;
            temp_low=v[i]+per_lim;
            temp_high=temp_low+vac_lim;
        }
        else if(v[i]>=temp_low && v[i]<=temp_high)
        {
            curr++;
            continue;
        }
        else if(v[i]<temp_low && (v[i]+per_lim)>=temp_low)
        {
            curr++;
            continue;
        }
        else if(v[i]>temp_high)
        {
            ans++;
            curr=1;
            temp_low=v[i]+per_lim;
