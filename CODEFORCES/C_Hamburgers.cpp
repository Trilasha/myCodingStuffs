...
ll b=0,s=0,c=0;
bool check(ll mid,vector<ll> &have,vector<ll> &cost,ll rupy)
{
    ll tot=0;
    ll req;
    ...
    //bread
    req=mid*b;
    tot+=max(0ll,(req-have[0])*cost[0]);


    //sausage
    req=mid*s;
    tot+=max(0ll,(req-have[1])*cost[1]);

    //cheese
    req=mid*c;
    tot+=max(0ll,(req-have[2])*cost[2]);
