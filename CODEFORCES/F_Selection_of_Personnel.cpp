...
struct nCr{
    ...
    ll invMod(ll a) {return power(a,md-2);}
    void pre() {
        fact[0] = 1 ;
        for(ll i = 1;i< maxx;++i) fact[i] = mul(i, fact[i-1]) ;
        ifact[maxx-1] = invMod(fact[maxx-1]) ; 
        for(ll i = maxx-1 ; i>0 ;--i) ifact[i-1] = mul(ifact[i], i) ;
    }
    nCr(ll _mxN, ll _M) {
        maxx = _mxN + 1; 
        md = _M ; 
...
void solve()
{
   ...
   ll ans=comb.C(n,5)+comb.C(n,6)+comb.C(n,7);
   cout<<ans<<endl;
}
