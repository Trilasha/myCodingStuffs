...
vector<ll> ind1(102),ind2(102);
ll qu=0;
void dfs(vector<ll> &temp,ll a,ll qu)
{
    temp[a]=1;
    for(ll i=1;i<=qu;++i)
    {
        ...
        if(ind1[i]<ind1[a] && ind1[a]<ind2[i])
        dfs(temp,i,qu);
        if(ind1[i]<ind2[a] && ind2[a]<ind2[i])
        dfs(temp,i,qu);
    }
