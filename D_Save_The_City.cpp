...
void find_path(ll x,ll y,ll x2,ll y2,ll cnt,vector<vector<ll>> &v)
{
    //cout<<x<<"-->"<<y<<endl;
   // cout<<cnt<<endl;
    if(x<0 || x>=n || y<0 || y>=m)
    ...
    if(x<n-1)
    find_path(x+1,y,x2,y2,min(cnt,v[x+1][y]),v);
    if(x>0)
    find_path(x-1,y,x2,y2,min(cnt,v[x-1][y]),v);
    if(y<m-1)
    find_path(x,y+1,x2,y2,min(cnt,v[x][y+1]),v);
    if(y>0)
    find_path(x,y-1,x2,y2,min(cnt,v[x][y-1]),v);
